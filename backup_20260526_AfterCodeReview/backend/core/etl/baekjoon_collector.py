...
class BaekjoonCollector:
    ...
    def _save_solve_history(self, problems):
        """수집한 문제를 SolveHistory에 저장 (중복 무시, bulk_create로 N+1 제거)"""
        from core.models import SolveHistory

        # 기존 문제 ID 일괄 조회 — 루프 안 get_or_create N번 → 쿼리 2번으로 축소
        existing_ids = set(
            SolveHistory.objects.filter(
                user=self.user,
        ...
        for p in problems:
            ...
            if not problem_id or problem_id in existing_ids:
                continue
        ...
        if to_create:
            try:
                SolveHistory.objects.bulk_create(to_create, ignore_conflicts=True)
                self.synced += len(to_create)
            except Exception as e:
                logger.warning(f"[Baekjoon] bulk_create 실패: {e}")
                self.errors += len(to_create)
