...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['reset']:
            self.stdout.write("🗑️  기존 문제 데이터 초기화 중...")
    ...
    def _load_problems(self, folder, target_role=None):
        ...
        for filename in sorted(files):
            ...
            for p in problems:
                JobProblem.objects.update_or_create(
                    ...
                    original_question_id=p['question_id'],
                    defaults={
        ...
        self.stdout.write(f"   ✅ 총 {total}문제 적재")
            ...
            for e in data.get('edges', []):
                try:
                    src = JobProblem.objects.get(
                        job_role=job_role,
                        original_question_id=e['source']
                    )
                    tgt = JobProblem.objects.get(
                        job_role=job_role,
                        original_question_id=e['target']
                    )
                    ...
                    self.stderr.write(
                        f"   ⚠️  문제 없음: {job_role} Q{e.get('source')}→Q{e.get('target')}"
                    )
